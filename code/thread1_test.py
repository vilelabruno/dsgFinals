   ...
   while not exitFlag:
      ...
      if not workQueue.empty():
         ...
         if ll[0] != "null":
            ...
            test["avgProdPrice"][test["index"] == data] = avgg 
            test["minProdPrice"][test["index"] == data] = minn 
            test["maxProdPrice"][test["index"] == data] = maxx 
            test["stdProdPrice"][test["index"] == data] = stdd 
      else:
         queueLock.release()
...
df = pd.DataFrame()
test["products"] = test["products"].fillna("["null"]")
...
nameList = list(range(len(test)))
queueLock = threading.Lock()
workQueue = queue.Queue(len(test)+1)
threads = []
threadID = 1
...
for t in threads:
   t.join()
pathout = "~/felipe/DSG-Finals/out/"
test.to_csv(pathout + "test_products.csv", index=False)
