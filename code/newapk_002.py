...
products = pd.read_csv("productid_category.csv")
...
# Number of events between the last purchase and the actual event
merge['is_purchase'] = 0
purchase = ['PURCHASE_PRODUCT_CAROUSEL', 'PURCHASE_PRODUCT_LP','PURCHASE_PRODUCT_LR', 'PURCHASE_PRODUCT_PA','PURCHASE_PRODUCT_SHOW_CASE', 'PURCHASE_PRODUCT_UNKNOW_ORIGIN']

for value in purchase:
	merge['is_purchase'][merge['type'] == value] = 1

max_moment = merge.groupby('sid')['index'].max().reset_index()
aux_mnt = merge[merge['is_purchase'] == 1]
buy_moment = aux_mnt.groupby('sid')['index'].max().reset_index()
max_moment.columns = ['sid', 'max_moment']
buy_moment.columns = ['sid', 'buy_moment']
max_moment = pd.merge(max_moment, buy_moment, on='sid', how='left', sort=False)
max_moment['diff_in_lines'] = max_moment['max_moment'] - max_moment['buy_moment']
max_moment = max_moment[['sid','diff_in_lines']]
merge.drop(['is_purchase'], axis=1, inplace=True)

# Number of produtcs added in basket between the actual event and the last purchase event
merge['is_basket'] = 0
basket   = ['ADD_TO_BASKET_CAROUSEL','ADD_TO_BASKET_LP','ADD_TO_BASKET_LR','ADD_TO_BASKET_PA','ADD_TO_BASKET_SHOW_CASE']
for value in basket:
	merge['is_basket'][merge['type'] == value] = 1
max_moment = merge.groupby('sid')['index'].max().reset_index()
max_moment.columns = ['sid', 'max_moment']
aux_mnt = merge[merge['is_basket'] == 1]
aux_mnt = pd.merge(aux_mnt, max_moment, on='sid', how='left', sort=False)
aux_mnt = pd.merge(aux_mnt, buy_moment, on='sid', how='left', sort=False)
aux_mnt = aux_mnt[aux_mnt['index'] < aux_mnt['max_moment']]
aux_mnt = aux_mnt[aux_mnt['index'] > aux_mnt['buy_moment']]
numberadd = aux_mnt.groupby('sid')['index'].count().reset_index()
numberadd.columns = ['sid','numberADDBasket']
f_train = pd.merge(f_train, numberadd, on='sid', how='left', sort=False)
f_test  = pd.merge(f_test, numberadd, on='sid', how='left', sort=False)
...
merge['time/quantity'] = merge['seconds'] / merge['quantity']

int_columns = ['nb_query_terms','rcount','quantity','orcount','seconds','time/quantity']
...
for col in cat_columns:
    merge[col], indexer = pd.factorize(merge[col])

for col in cat_columns:
	encoding = merge.groupby('sid')[col].mean().reset_index()
	encoding.columns = ['sid','vect_mean_'+col]
	f_train = pd.merge(f_train, encoding, on='sid', how='left', sort=False)
	f_test  = pd.merge(f_test, encoding, on='sid', how='left', sort=False)

for col in cat_columns:
	encoding = merge.groupby('sid')[col].max().reset_index()
	encoding.columns = ['sid','vect_max_'+col]
	f_train = pd.merge(f_train, encoding, on='sid', how='left', sort=False)
	f_test  = pd.merge(f_test, encoding, on='sid', how='left', sort=False)

for col in cat_columns:
	encoding = merge.groupby('sid')[col].min().reset_index()
	encoding.columns = ['sid','vect_min_'+col]
	f_train = pd.merge(f_train, encoding, on='sid', how='left', sort=False)
	f_test  = pd.merge(f_test, encoding, on='sid', how='left', sort=False)
...
f_train.to_csv('train_xgb.csv', index=False)
f_test.to_csv('test_xgb.csv', index=False)
...
submission.to_csv("newapk_001.csv", index=False)